...
class UserService:
    ...
    def validate(self, username, password, password_confirmation):
        ...
        if len(password) < 8:
            raise UserInputError("Invalid password: Password must be atleast 8 characters")

        if re.search("[^a-z^A-Z]", password) is None:
            raise UserInputError("Invalid password: Password must contain atleast one special character or number")
